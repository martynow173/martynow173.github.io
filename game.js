(function(){
    ...
    var Ball = function () {
		return {
			...
			yspeed: 3,
			xspeed: 3,
			bounce: 1.1, //коэфицент упругости - для ускорения шарика после отскока
			render: function (ctx) {
				ctx.beginPath();
	...
	//Платформы для отбивания шарика
	...
	var Game = function () {
		...
		//Параметры с которыми будет окно игры
		this.params = {
			width: 800,
			height: 500,
	...
	Game.prototype = {
		...
		startGame: function () {
			...
			this.objects.bracket2.color = '#919191';
		...
		keyDownEvent: function (event) {
			var kCode = event.keyCode;
				//левая вниз "s"
				if(kCode === 83) {
					game.objects.bracket1.y = game.objects.bracket1.y + 2*game.objects.bracket1.speed;
				} 
				//левая вверх "w"
				if(kCode === 87) {
					game.objects.bracket1.y = game.objects.bracket1.y - 2*game.objects.bracket1.speed;
				} 
				//правая вниз num pad: "5"
				if(kCode === 101) {
					game.objects.bracket2.y = game.objects.bracket2.y + 2*game.objects.bracket2.speed;
				} 
				//правая вверх num pad: "8"
				if(kCode === 104) {
					game.objects.bracket2.y = game.objects.bracket2.y - 2*game.objects.bracket2.speed;
				} 
				//E - рестарт шарика
